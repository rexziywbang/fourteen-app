...
import { getCrushForSender, getCrushMessage } from "@/lib/db";
...
export default async function SentCrushPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ new?: string }> }) {
  ...
  const crush = getCrushForSender(user.id, id);
  if (!crush) notFound();
  return (
    <main className="detail-page">
      <header className="detail-header"><Link href="/home" aria-label="Back home">←</Link><Brand compact /><span /></header>
      {justSent && <div className="sent-success"><span>♥</span><p className="eyebrow">Your one brave thing</p><h1>It’s out there.</h1><p>We’ll tell {crush.recipientFirstName} without telling them who. You can watch the fuse from here.</p></div>}
      <section className="sender-summary"><p className="eyebrow">Sent to</p><h1>{crush.recipientFirstName} {crush.recipientLastName}</h1><blockquote>“{getCrushMessage(crush.messageId)}”</blockquote><div className="privacy-chip">Your identity is still private</div></section>
