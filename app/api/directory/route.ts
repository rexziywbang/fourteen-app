...
import { consumeRateLimit, searchDirectory } from "@/lib/db";
...
export async function GET(request: NextRequest) {
  ...
  const rate = consumeRateLimit(user.id, "search_directory", 30, 60);
  ...
  return NextResponse.json(searchDirectory(user.id, parsed.data), { headers: { "Cache-Control": "no-store" } });
}
