...
import { blockFromPickAction } from "@/app/actions";
import { BottomNav } from "@/components/bottom-nav";
import { Brand } from "@/components/brand";
import { Fuse } from "@/components/fuse";
import { getCompliments, getCrushMessage, getHomeData, getNotifications } from "@/lib/backend";
...
function notificationCopy(kind: unknown, payload: Record<string, unknown>) {
  ...
  if (kind === "quiet_close") return "The window closed quietly. Your next one unlocks Monday.";
...
export default async function HomePage({ searchParams }: { searchParams: Promise<{ welcome?: string }> }) {
  ...
  const [data, compliments, notifications] = await Promise.all([
    getHomeData(user.id),
    getCompliments(user.id),
    ...
    <main className="app-shell">
      ...
      <section className="feed">
        ...
        <section className="feed-zone">
          ...
          {data.received.length ? data.received.map((item) => <Link className="crush-card crush-card--received" href={item.status === "mutual" || item.status === "revealed" ? `/reveal/${item.id}` : `/crush/${item.id}`} key={String(item.id)}><div className="card-kicker"><span className="pulse-dot" /> {item.status === "mutual" ? "It’s mutual" : item.status === "revealed" ? "They said yes" : "Someone has a crush on you"}</div><blockquote>“{getCrushMessage(Number(item.message_id))}”</blockquote><Fuse lit={Number(item.hints_unlocked)} /><div className="card-meta"><span>Day {15 - daysLeft(item.expires_at)} of 14 · {String(item.hints_unlocked)} hint unlocked</span><strong>{item.status === "mutual" || item.status === "revealed" ? "Reveal →" : `${daysLeft(item.expires_at)}d left →`}</strong></div></Link>) : <div className="empty-card"><span className="empty-moon">☾</span><h3>Nothing yet.</h3><p>Someone’s probably working up the nerve.</p></div>}
        </section>

        <section className="feed-zone">
          <div className="zone-title"><h2>Your move</h2></div>
          {data.sent.length ? data.sent.map((item) => <Link className="sent-card" href={item.status === "mutual" || item.status === "revealed" ? `/reveal/${item.id}` : `/sent/${item.id}`} key={String(item.id)}><div className="sent-card__copy"><span className="eyebrow">{item.status === "mutual" ? "It’s mutual" : item.status === "revealed" ? "Revealed" : "Fuse burning"}</span><h3>{String(item.recipient_first_name)} {String(item.recipient_last_name)}</h3><Fuse lit={Number(item.hints_unlocked)} /><p>{String(item.hints_unlocked)} of 14 hints unlocked</p></div><div className="countdown"><strong>{item.status === "mutual" || item.status === "revealed" ? "✦" : daysLeft(item.expires_at)}</strong><span>{item.status === "mutual" || item.status === "revealed" ? "open" : "days"}</span></div></Link>) : <Link href="/send" className="send-cta"><span className="cta-heart">♥</span><div><p className="eyebrow">One brave thing</p><h3>Send this week’s crush</h3><p>You get one. Make it honest.</p></div><span>→</span></Link>}
        </section>
        ...
        <section className="feed-zone"><div className="zone-title"><h2>Kind things said</h2><span>{compliments.length}</span></div>{compliments.length ? <div className="compliment-list">{compliments.map((item) => <article key={String(item.id)}><span>✦</span><div><small>Someone said</small><p>“{String(item.prompt_text)}”</p><details className="compliment-safety"><summary>…</summary><div><p>They’ll simply stop appearing. No one is told.</p><form action={blockFromPickAction}><input type="hidden" name="pickId" value={String(item.id)} /><button>Block this person</button></form></div></details></div></article>)}</div> : <div className="empty-compliments"><span>✦</span><p>Compliments land here—without the picker’s name.</p></div>}</section>
